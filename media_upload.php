<?php
/** formerly resize_images.php **/


function make_image($cur_dir, $new_dir, $max_width, $max_height, $file_name='', $file_type, $new_imagename, $pres_ratio=true, $ext_prt='')
{
	$rt = false;
    $img_name = $file_name;
    $img_type = $file_type;
    ...
    if ($img_type == "image/x-icon")
    {
        //var_dump_p($cur_dir, $img_name, $new_dir, $new_imagename.'.'.$ext_prt); exit;
        $rt = move_uploaded_file($cur_dir.$img_name, $new_dir.$new_imagename.'.'.$ext_prt);
        return $rt;
    }
 ...
 *
 * $img_width = fix width to resize; set to '0' to prevent resize; Use 'max:{size}' to only resize if larger than {size}
 * $img_height = fix height to resize; set to '0' to prevent resize; Use 'max:{size}' to only resize if larger than {size}
 *
 ...
 *  $new_p_files = $sql_part_2 = array();
    ...
    $new_p_files = upload_img_rs_ar('p_file', 0, 0, $up_path, 'Image', '', '', 'PWR_MSG_GLOBAL', false, $copy_data);
...
function upload_img_rs_ar($field_name, $img_width, $img_height, $up_loc, $img_title, $insert_name_part='', $fix_height='', $error_sess_name='PWR_MSG_GLOBAL', $images_only=false, $copies=array())
{
    $rtn_ar = array();
    if(isset($_FILES[$field_name]) && @is_array($_FILES[$field_name]) && @is_array($_FILES[$field_name]['tmp_name']))
    {
        for($i=0; $i<count($_FILES[$field_name]['tmp_name']); $i++)
        {
            ...
            if($m_type=='video')
            {
                ...
                $new_name = upload_media($field_name, $up_loc, $img_title, $insert_name_part, $error_sess_name, $i);
            }
 ...
 *
 * $field_name = Image field name
 * $img_width = fix width to resize; set to '0' to prevent resize; Use 'max:{size}' (e.g. max:200) to only resize if larger than {size}
 * $img_height = fix height to resize; set to '0' to prevent resize; Use 'max:{size}' to only resize if larger than {size}
 *
 * $up_loc = upload location for the image. Like '../img/clients'
 * $img_title = Image Title (mainly used for errors)
...
function upload_img_rs($field_name, $img_width, $img_height, $up_loc, $img_title, $insert_name_part='', $fix_height='', $error_sess_name='PWR_MSG_GLOBAL', $array_index=0, $copies=array())
{
    ...
    if(is_uploaded_file($tmp_name))
    {
        ...
        if(stristr($img_width, 'max:')!=false){
            $img_width = (int)substr($img_width, 4);
            if(($img_width>0) && ($g_width>$img_width)) $g_width = $img_width;
        }
        else {
            if(($img_width>0) && (($g_width>$img_width) || ($g_width<($img_width-1)))) $g_width = $img_width;
        }

        if(stristr($img_height, 'max:')!=false){
            $img_height = (int)substr($img_height, 4);
            if(($img_height>0) && ($g_height>$img_height)) $g_height = $img_height;
        }
        else {
            if(($img_height>0) && (($g_height>$img_height) || ($g_height<($img_height-1)))) $g_height = $img_height;
        }
        //var_dump($img_width, $img_height, $g_width, $g_height); die();
        #-
        ...
        #/ Create, Resize & Save image
        $ext_prt = strtolower($t1[count($t1)-1]);
        $ret = false;
        $ret = make_image($tmp_name, $up_loc, $g_width, $g_height, '', $type, $new_c_image, $pres_ratio, $ext_prt);
        ...
        {
            $ext_prt = str_replace(array('jpeg', 'jpeg'), 'jpg', $ext_prt);
            $new_c_image.= '.'.$ext_prt;
        }
    }
...
function upload_media($field_name, $up_loc, $img_title, $insert_name_part='', $error_sess_name='PWR_MSG_GLOBAL', $array_index=0)
{
    global $_FILES, $_SESSION;
...
function upload_img_external($external_url, $save_as, $img_width, $up_loc, $img_title, $insert_name_part='', $fix_height='', $error_sess_name='PWR_MSG_GLOBAL')
{
    global $_SESSION;
