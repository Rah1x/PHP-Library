...
{
    public $GET = array();
    public $query, $lim, $start, $tot, $first_page, $tot_pg, $nxt_pg;
     ...
     **/
    function query_limits($fast=false)
    {
        ## Set Limits
        ...
        if(!isset($this->GET['tot']))
        {
            ...
            {
                $r2 = @mysql_exec("SELECT count(*) cx FROM ({$this->query}) ax", 'single');
                if(is_array($r2) && count($r2)>0){$this->tot = (int)$r2['cx'];}
                //var_dump($r2, $this->tot); die('x');
            }
        }
        else
        {
            $this->tot = (int)$this->GET['tot'];
        }
        ##--


        if($this->tot > 0){
        $this->tot_pg = @ceil($this->tot/$this->lim);
        }

        $this->nxt_pg = $this->GET['page']+1;
        if($this->nxt_pg > $this->tot_pg)
        $this->nxt_pg = $this->tot_pg;
