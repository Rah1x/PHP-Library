<link rel="stylesheet" href="<?=DOC_ROOT?>assets/css/post_login.css?v=1.2" type="text/css" />

<div class="head_boxes" style="margin-top:8px;">
...
<div class="searchhead">
    ...
    <form name="search_form" id="search_form" action="<?=DOC_ROOT?>ecosystem/search" method="post">
        ...
        <div class="searinpt"><input name="sk" type="text" value="<?php echo isset($_POST['sk']) ? $_POST['sk'] : '';?>" /></div>
...
<ul>
    ...
    <li>
        ...
        */ ?>
        ...
        <ul class="marginulin marginuliniph pad_1 nav_1">
            ...
            <?php
            $def_prof_pic = DOC_ROOT."assets/images/ep.png";
            ...
            foreach($my_msgs_list as $my_msgs_v)
            {
                ...
                if(!empty($my_msgs_v['from_user_id']) && isset($my_msg_users_info[$my_msgs_v['from_user_id']]))
                {
                    ...
                    if(empty($img_vp))$img_v = $def_prof_pic;
                    else $img_v = DOC_ROOT."user_files/prof/{$my_msgs_v['from_user_id']}/{$img_vp}";
                    $img_v = @substr_replace($img_v, '_th.', @strrpos($img_v, '.'), 1);
                }
                ...
                onclick=\"$.visit_msgT('{$my_msgs_v['thread_id']}');\">";
                    ...
                    echo "<div class=\"notsmallp msgx_par\">";
                        ...
                        #/ Profile Names
                        $m_name = array(); $nm_i = 0;
                        if(is_array($members_ar) && count($members_ar)>0)
    ...
    <li>
        ...
        $notifs_count = (int)@count_notification($user_idc, $user_info_c);

        //$notifs = @read_notification($user_idc); //old method not using due to inefficiency
        $notifs = get_notifications($user_idc, $user_info_c, '', '', 8, true);
        $notifs_l = @$notifs[0];
        $notifs_ui = @$notifs[1];
        //var_dump_p($notifs); die();
        ?>
        ...
        <ul class="marginulin marginuliniph pad_1 nav_2">
            ...
            foreach($notifs_l as $notif_v)
            {
                ...
                if(!empty($notif_data['from_user_id']) && !empty($from_user_i))
                {
                    ...
                    if(empty($img_vp))$img_v = $def_prof_pic;
                    else $img_v = DOC_ROOT."user_files/prof/{$notif_data['from_user_id']}/{$img_vp}";
                    $img_v = @substr_replace($img_v, '_th.', @strrpos($img_v, '.'), 1);
                }
                ...
                onclick=\"$.process_notifi('{$notif_data['id']}', '', '{$notif_data['notif_type']}');\" target=\"{$target}\">";
    ...
    <li>
        <div class="iconsusrnav">
            ...
            <?php $border_pfr=''; if($prf_pic_th=='/assets/images/ep_th.png'){$border_pfr='border:solid 1px #E8E8E8;';} ?>
            <img src="<?=@$prf_pic_th?>" style="width:35px; height:35px; <?=$border_pfr?>" class="round_borders" title="<?=@($user_info_c['first_name'].' '.$user_info_c['last_name'])?>" /></a>
        </div>

        <ul class="marginuliniph pad_2 width_300 nav_3">
            <?php #/* ?><li class="title"><h2>My Account ::</h2></li>
            <li class="brdrnonebot"><a href="<?=DOC_ROOT?>ecosystem/">My News Feed</a></li>
            <li class="brdrnonebot"><a href="<?=DOC_ROOT?>estore/shop/landingpage/">My Marketplace</a></li>
            ...
            <li class="brdrnonebot"><a href="<?=DOC_ROOT?>my-account" style="font-weight:bold;">Account<?php if(@$user_info_c['is_basic']=='0'){echo ' & Payment Info';}else{echo ' Settings';} ?></a></li><?php #*/ ?>
            <?php if(@$user_info_c['allow_upgrade']=='true'){ ?><li class="brdrnonebot"><a href="<?=DOC_ROOT?>upgrade" style="color:#CF2600; font-weight:bold">UPGRADE Membership</a></li><?php } ?>
...
<script type="text/javascript" src="<?=DOC_ROOT?>assets/js/head_boxes.js?v=1.1"></script>