...
class global_notif
{
    ...
    function global_notif($template_id, $object, $POST, $created_on)
    {
        ...
        $this->pack_id = 0;
        $this->fixed_users = false;
        if(stristr($POST['user_ids'], 'P_')!=false)
        {
            $pack_id_r = @explode('P_', @$POST['user_ids']);
            $pack_id = (int)@$pack_id_r[1];
            $this->pack_id = $pack_id;
        } else if($POST['user_ids']!='*') {
        ...
        $this->heading = "New Notification from CollaborateUSA&#46;com";

        include_once('../../includes/PHPMailer/class.phpmailer.php');
        $this->mailx = new PHPMailer;
        $this->mailx->isSMTP();

        if(in_array($_SERVER['SERVER_NAME'], array('cusa-local', 'cusa-local.com', 'cusa-dev', 'localhost'))==false){ //SERVER
        include_once('../../includes/PHPMailer/class.smtp.php');
        $this->mailx->Host = SMTP_HOSTX;
        ...
        $this->mailx->Username = SMTP_USERX;
        $this->mailx->Password = SMTP_PASSWORDX;
        ...
        $this->mailx->setFrom(SMTP_USERX, 'CollaborateUSA.com');
        $this->mailx->addReplyTo(SMTP_USERX, 'CollaborateUSA.com');
        $this->mailx->ReturnPath = SMTP_USERX;
    ...
    function send_notifs($st)
    {
        ...
        else
        {
            foreach($recipients as $rv)
            {
                ...
                #/ Setup Body
                $body_in = notification_email('', $rv, $notification, $this->created_on, @$POST['visit_url']);
